...
const translations = {
    en: {
        ...
        about_text: "My focus lies in discovering the <span class='text-glow-red'>'leverage points'</span> hidden within user experiences, using minimal effort to drive maximum impact.",
        ...
        view_pdf_btn: "VIEW FULL PORTFOLIO (PDF)",
    ...
    zh: {
        ...
        nav_contact: "联络",
        cover_title: "产品设计学生",
        about_text: "我专注于发现隐藏于用户体验中的<span class='text-glow-red'>'翘点'</span>,用最小的代价撬动最大的改变",
    ...
    ja: {
        ...
        about_text: "ユーザー体験の中に隠された<span class='text-glow-red'>「レバレッジ・ポイント」</span>を見極め、最小の労力で最大のインパクトを引き出すことに注力しています。",
    ...
    if (canvas) {
        ...
        function render() {
            ...
            if (contactSection) {
                ...
                // Shifted significantly upwards: target resting point is now negative (above screen top if fully scrolled)
                bottomGroupBaseY = (height * -0.2) + (rect.top * 1.0);
            }
            ...
            if (bottomGroupBaseY > -3000 && bottomGroupBaseY < height + 800) {
                // Opacity fades in as it enters, trigger even earlier
                const entranceOpacity = Math.min(1, Math.max(0, (height * 2.0 - bottomGroupBaseY) / 500));
            ...
            .then(data => {
                if (data && data.posts && data.posts.length > 0) {
                    const postsHTML = data.posts.slice(0, 6).map((post, index) => {
                        ...
                        return `<a href="${post.permalink}" target="_blank" class="photo-item placeholder-image bg-dark-gray" style="aspect-ratio: 1/1; background-image: url('${imgUrl}'); background-size: cover; background-position: center; display: block; border: 1px solid var(--color-border);"></a>`;
                    }).join('');
